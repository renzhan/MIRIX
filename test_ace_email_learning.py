...
async def test_ace_email_learning():
    """测试ACE邮件学习（直接使用严格结构化格式）"""
    
    logger.info("=" * 60)
    logger.info("开始 ACE 邮件学习测试（结构化版 + 新评估标准）")
    logger.info("=" * 60)
...
第四步：与Joliet团队接洽，了解他们的技术能力，并评估是否有更好的解决方案

第五步：整合B-Solutions和Unis关于API处理和订单类型区分的反馈

第六步：定期与Frances Parro Belleza和Michael Jan Francisco同步项目进展，确保团队理解一致
...
"""
    ...
    # 5. 构造训练样本
    logger.info("\n[5/5] 提取邮件主题并构造question...")
    email_topic_1 = extract_email_topic(sample_email_1, llm_client)
    
    # 基于提取的主题构造question
    specific_question_1 = f"{email_topic_1}需要联系哪些人？需要检查哪些系统？需要执行哪些操作？"
    logger.info(f"✓ 动态question: {specific_question_1}")
    
    # 构造训练样本
    training_samples = [
        Sample(
            question=specific_question_1,  # 使用动态提取的主题
            context=json.dumps({
                "original_email": sample_email_1,
                "workflow_result": workflow_result_1,
                "history": ""
            }, ensure_ascii=False),
            ground_truth=ground_truth_1,  # 直接使用严格结构化格式
            metadata={"email_type": "technical_support", "sample_id": 1, "extracted_topic": email_topic_1}
        )
    ]
    
    logger.info(f"✓ 准备了 {len(training_samples)} 个训练样本")
    
    # 6. 开始训练
    logger.info("\n" + "=" * 60)
    logger.info("开始ACE训练（新评估标准）")
    logger.info("=" * 60)
    playbook = Playbook()
